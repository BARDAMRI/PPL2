...
export const parseL31SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op === "class" ? parseClassExp(first(params),first(rest(params))):
...
const parseClassExp = (fields: Sexp, bindings: Sexp): Result<ClassExp> =>{
    if((!isGoodBindings(bindings)) || (!isArray(fields)) || (!allT(isString, fields))) 
        return makeFailure(`Invalid vars or methods for ClassExp`);  
    return bind(mapResult(parseL31CExp, map(second,bindings)),(exp: CExp[])=> makeOk(makeClassExp(map(makeVarDecl,fields),zipWith(makeBinding,map(m=>m[0],bindings),exp))));
...
const unparseClassExp = (cl : ClassExp) : string => 
    `(class (${map((fiel: VarDecl) => fiel.var , cl.fields).join(" ")}) (${map((b: Binding) => `(${b.var.var} ${unparseL31(b.val)})`,cl.methods).join(" ")}))`
